...
const EnrolledCourses = () => {
  ...
  useEffect(() => {
    const fetchEnrolledCourses = async () => {
      try {
        ...
        alert(err.response?.data?.message || "Failed to fetch enrolled courses");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-800 text-white">
      <h2 className="text-3xl font-bold text-center mb-8">Your Enrolled Courses</h2>
      ...
      {courses.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course) => {
            ...
            return (
              <div key={course._id} className="bg-gray-700 shadow-lg rounded-lg overflow-hidden">
                <img
                ...
                <div className="p-4">
                  ...
                  <p className="text-blue-400 font-medium">Category: {course.category}</p>
                  <p className="text-green-400 font-medium mb-2">Price: ${course.price}</p>
